...
function getFinals(array) {
    const finalsTeams = array.filter(function(obj1) {
        return obj1.Stage == 'Final'
    });
    return finalsTeams;
    }
 
console.log('test', getFinals(fifaData));
...
function getYears(array,callBack) {
    const getfinalsdataset = callBack(array); 
    const years = getfinalsdataset.map(newfunction);
   return years
}
   function newfunction(obj2){
    return obj2.Year
   }
...
function getWinners(array,cb) {
    ...
    return cb(array).map(winnersfunction);
}

function winnersfunction(data){
